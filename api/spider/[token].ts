/**
 * Self-contained Vercel serverless function: GET /s/:token → Kayak hotels 302.
 *
 ...
 * The local Vite dev equivalent lives in server/spider/spiderCore.ts — keep in sync.
 */
...
const isValidSpiderToken = (provided: string | undefined | null): boolean => {
...
type SpiderKayakRedirectResult =
...
const runSpiderKayakRedirect = async (): Promise<SpiderKayakRedirectResult> => {
  const destinationQuery = pickRandomSpiderDestination();
  const dateRange = pickRandomSpiderDateRange();
