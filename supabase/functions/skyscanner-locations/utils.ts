...
export function getCacheKey(searchTerm: string, market: string, locale: string): string {
  return `skyscanner_locations_${market}_${locale}_${searchTerm.toLowerCase()}`;
}
...
/** Metro / city IATA codes that Skyscanner substring search mishandles (e.g. NYC → Swedish towns). */
